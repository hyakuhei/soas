...
logger = logging.basicConfig(
    filename=f"{__name__}-{timestr}", encoding="utf-8", level=logging.DEBUG
)
...
def _gatherScripts():
    ...
    scripts = [
        f for f in listdir(SCRIPTS) if (isfile(join(SCRIPTS, f)) and f.endswith(".py"))
    ]
...
def main():
    menu = ConsoleMenu(
        "Safe Operator Access Shell",
        "Activities are logged. Seek permission to go 'off script'",
    )
    ...
    for m in loaded_modules.keys():
        ...
        if declaration["behaviors"]["mutating"] == True:
            t = color(f"{declaration['name']}", fg="yellow")
        if declaration["behaviors"]["dangerous"] == True:
            t = color(f"{declaration['name']}", fg="red")

        menu.append_item(FunctionItem(t, _run, args=[loaded_modules[m].entrypoint]))
        # print(f"{m} : {loaded_modules[m].declaration()}")

    menu.append_item(
        CommandItem(color("Run a console command", fg="purple"), "/bin/.sh")
    )
    menu.start()
    menu.join()
